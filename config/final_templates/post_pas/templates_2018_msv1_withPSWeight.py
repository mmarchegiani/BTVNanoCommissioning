...
cfg =  {
    "dataset" : {
        "jsons": [#"datasets/MC_QCD_MuEnriched_RunIISummer20UL_redirector.json",
                  "datasets/MC_QCD_HT_RunIISummer20UL_redirector.json",
    ...
    "output"   : "output/pocket_coffea/final_templates/post_pas/templates_2018_NanoAODv9_withPSWeight",
    "workflow_options" : {
   ...
   "variables":
    {
        "FatJetGood_logsumcorrSVmass": HistConf(
            [ Axis(coll="FatJetGood", field="logsumcorrSVmass", label=r"log($\sum({m^{corr}_{SV}})$ / GeV)", bins=42, start=-2.4, stop=6) ]
        ),
        ...
        "FatJetGood_logsv1mass": HistConf(
            [ Axis(coll="FatJetGood", field="logsv1mass", label=r"log($m_{SV1, d_{xy}sig max}$ / GeV)", bins=42, start=-2.4, stop=6) ]
        ),
        "FatJetGood_logsv1mass_tau21": HistConf(
            [ Axis(coll="FatJetGood", field="logsv1mass", label=r"log($m_{SV1, d_{xy}sig max}$ / GeV)", bins=42, start=-2.4, stop=6),
              Axis(coll="FatJetGood", field="tau21", label=r"$\tau_{21}$", type="variable", bins=[0, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 1]) ]
        ),
    },
